...
class dumper {
	...
	function backup() {
		...
		$buttons = "<span ID=save STYLE='display: none;'>Бекап БД успешно создан в папке /backup/</span> &nbsp; <INPUT ID=back TYPE=button VALUE='Закрыть' DISABLED onClick=\"window.close();\">";
		...
		echo tpl_l("Создание файла с резервной копией БД:");
	...
	function restore(){
		...
		if (!isset($_POST)) {$this->main();}
		@set_error_handler("SXD_errorHandler", E_ALL ^ E_NOTICE);
		$buttons = "<INPUT ID=back TYPE=button VALUE='Закрыть' DISABLED onClick=\"window.close();\">";
	...
	function main(){
		...
		$buttons = "<INPUT TYPE=submit VALUE=Применить><INPUT TYPE=button VALUE=Закрыть onClick=\"window.close();\">";
