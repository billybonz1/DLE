...
function check_login($username, $md5_password, $post = true) {
	global $member_id, $db, $user_group, $lang, $_IP, $config;
	...
	if ( !$result ) if ($config['login_log']) $db->query( "INSERT INTO " . PREFIX . "_login_log (ip, count, date) VALUES('{$_IP}', '0', '".time()."') ON DUPLICATE KEY UPDATE count=count+1, date='".time()."'" );
...
function build_js($js) {
	...
	if ($config['js_min'] AND version_compare(PHP_VERSION, '5.1.0', '>') ) {
		...
		$js_array[] = "<script type=\"text/javascript\" src=\"{$config['http_home_url']}engine/classes/min/index.php?charset={$config['charset']}&amp;g=admin&amp;3\"></script>";
