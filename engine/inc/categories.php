...
=====================================================
 ƒ‡ÌÌ˚È ÍÓ‰ Á‡˘Ë˘ÂÌ ‡‚ÚÓÒÍËÏË Ô‡‚‡ÏË
=====================================================
 ‘‡ÈÎ: categories.php
-----------------------------------------------------
 Õ‡ÁÌ‡˜ÂÌËÂ: ÛÔ‡‚ÎÂÌËÂ Í‡ÚÂ„ÓËˇÏË
=====================================================
*/
...
if( ! is_array( $cat_info ) ) {   
	...
	if(!isset($_REQUEST['num']) || $_REQUEST['num'] == '0')  {
		...
		$n = "WHERE num='{$_REQUEST['num']}'";
	}
	$db->query( "SELECT * FROM " . PREFIX . "_category $n ORDER BY posi ASC" );   
...
if(!isset($_REQUEST['after'])) {
	header("Location: " . $_SERVER['REQUEST_URI'] . "&num=" . $nnum . "&after");
}
$result = "";
...
if( $_REQUEST['action'] == 'sort' ) {
	...
	@unlink( ENGINE_DIR . '/cache/system/category.php' );
	header( "Location:$PHP_SELF?mod=categories{$llk}" );
}
...
if( $action == "add" ) {
	...
	$chRes = $db->query("SELECT num FROM " . PREFIX . "_category WHERE id='$category'");
	$chMyr = $db->get_array($chRes);
	if($razdel != "" && $chMyr['num'] == $razdel || $category == 0) {
	...
	$sql_result = $db->query( "SELECT alt_name FROM " . PREFIX . "_category" );
	
	while ( $row = $db->get_row( $sql_result ) )
	if($nnum != 5) {
		if( $row['alt_name'] == $alt_cat_name ) {
			msg( "error", $lang['cat_error'], $lang['cat_eradd'], "?mod=categories{$llk}" );
		}
	}
	
	$db->query( "INSERT INTO " . PREFIX . "_category (parentid, name, alt_name, icon, skin, descr, keywords, news_sort, news_msort, news_number, short_tpl, full_tpl, metatitle, num) values ('$category', '$cat_name', '$alt_cat_name', '$cat_icon', '$skin_name', '$description', '$keywords', '$news_sort', '$news_msort', '$news_number', '$short_tpl', '$full_tpl', '$meta_title', '$razdel')" );
	
	@unlink( ENGINE_DIR . '/cache/system/category.php' );
	clear_cache();
	
	msg( "info", $lang['cat_addok'], $lang['cat_addok_1'], "?mod=categories{$llk}" );
	} else {
		msg( "error", "–‡Á‰ÂÎ ‚˚·‡Ì ÌÂ‚ÂÌÓ", "–‡Á‰ÂÎ ‚˚·‡Ì ÌÂ‚ÂÌÓ", "?mod=categories{$llk}" );
	}
} 
// ********************************************************************************
// ”‰‡ÎÂÌËÂ Í‡ÚÂ„ÓËË
...
elseif( $action == "remove" ) {
	...
	if( ! $catid ) {
		msg( "error", $lang['cat_error'], $lang['cat_noid'], "$PHP_SELF?mod=categories{$llk}" );
	}
	...
	if( $row['count'] ) {
		...
		if( is_array( $_REQUEST['new_category'] ) ) {
			if( ! in_array( $catid, $new_category ) ) {
				...
				msg( "info", $lang['cat_delok'], $lang['cat_delok_1'], "?mod=categories{$llk}" );
			}
		}
		
		msg( "info", $lang['all_info'], "<form action=\"\" method=\"post\">{$lang['comm_move']} <select name=\"new_category[]\" class=\"cat_select\" align=\"absmiddle\" multiple>" . CategoryNewsSelection( 0, 0 ) . "</select> <input class=\"edit\" type=\"submit\" value=\"{$lang['b_start']}\"></form>", "$PHP_SELF?mod=categories{$llk}" );
...
// ********************************************************************************
// –Â‰‡ÍÚËÓ‚‡ÌËÂ Í‡ÚÂ„ÓËË
// ********************************************************************************
elseif( $action == "edit" ) {
	
	echoheader( "options", $lang['cat_head'] );
	$hidden = "<input type='hidden' name='num' value='" . $_REQUEST['num'] . "' />";
	...
	if( ! $row['id'] ) msg( "error", $lang['cat_error'], $lang['cat_noid'], "$PHP_SELF?mod=categories{$llk}" );
...
<table width="100%">
    ...
    <tr>
        ...
        <td><input class="edit" type="text" name="short_tpl" value="{$row['short_tpl']}">.tpl<a href="#" class="hintanchor" onMouseover="showhint('{$lang[cat_s_tpl_hit]}', this, event, '250px')">[?]</a></td>
    </tr>
    <tr>
...
elseif( $action == "doedit" ) {
	...
	if( in_array( $parentid, get_sub_cats( $catid ) ) ) {
		msg( "error", $lang['cat_error'], $lang['cat_noparentid'], "$PHP_SELF?mod=categories{$llk}" );
	}
	...
	msg( "info", $lang['cat_editok'], $lang['cat_editok_1'], "$PHP_SELF?mod=categories{$llk}" );
}
// ********************************************************************************
// List all Categories
// ********************************************************************************
...
if(!isset($_REQUEST['num']) || $_REQUEST['num'] == '0') {
$razdel  = "<tr>
				<td style='padding:4px;'>–‡Á‰ÂÎ:</td>
				<td>";
$razdel .= "<select name='razdel'>";
$razdel .= "<option value='0'></option>";
$razdel .= "<option value='1'>œ–Œƒ” ÷»ﬂ</option>";
$razdel .= "<option value='2'>—’≈Ã€ Ë —œŒ—Œ¡€ œŒ –€“»…</option>";
$razdel .= "<option value='3'>œ–¿…—€ Ë  ŒÀ≈–Œ¬ ¿</option>";
$razdel .= "<option value='4'>–≈ ŒÃ≈Õƒ¿÷»» Ë Ã¿Àﬂ–Õ¿ﬂ ÿ ŒÀ¿</option>";
$razdel .= "<option value='5'>œŒ œ–Œ»«¬Œƒ»“≈ÀﬂÃ</option>";
$razdel .= "</select>";
$razdel .= "</td></tr>";
} else {
$razdel = "<input type='hidden' name='razdel' value='{$_REQUEST['num']}' />";
}


if($_REQUEST['num'] == '1'){$sel1 = "selected";}
if($_REQUEST['num'] == '2'){$sel2 = "selected";}
if($_REQUEST['num'] == '3'){$sel3 = "selected";}
if($_REQUEST['num'] == '4'){$sel4 = "selected";}
if($_REQUEST['num'] == '5'){$sel5 = "selected";}
if($_REQUEST['num'] == '6'){$sel6 = "selected";}

$chrazdel  = "<form action='/admin.php?mod=categories' method='post'>";
$chrazdel .= "<select name='num' onchange='submit()'>";
$chrazdel .= "<option value='1' {$sel1}>œ–Œƒ” ÷»ﬂ</option>";
$chrazdel .= "<option value='2' {$sel2}>—’≈Ã€ Ë —œŒ—Œ¡€ œŒ –€“»…</option>";
$chrazdel .= "<option value='3' {$sel3}>œ–¿…—€ Ë  ŒÀ≈–Œ¬ ¿</option>";
$chrazdel .= "<option value='4' {$sel4}>–≈ ŒÃ≈Õƒ¿÷»» Ë Ã¿Àﬂ–Õ¿ﬂ ÿ ŒÀ¿</option>";
$chrazdel .= "<option value='5' {$sel5}>œŒ œ–Œ»«¬Œƒ»“≈ÀﬂÃ</option>";
$chrazdel .= "<option value='6' {$sel6}>¬»ƒ≈Œ ”–Œ » ñ Ã¿—“≈–  À¿——</option>";
$chrazdel .= "</select>";
$chrazdel .= "</form>";

$skinlist = SelectSkin( '' );
echo $_REQUEST['razdel'];
echo <<<HTML
...
<table width="100%">
    <tr>
        <td bgcolor="#EFEFEF" height="29" style="padding-left:10px;">
			<div class="navigation" style='float:left;'>{$lang['cat_add']}</div>
			<div style='float:right;'>{$chrazdel}</div>
		</td>
    </tr>
</table>
...
<option value="ASC">{$lang['opt_sys_mplus']}</option>
    ...
    <tr>
        <td style="padding:4px;">{$lang['cat_s_tpl']}</td>
        <td><input class="edit" type="text" name="short_tpl">.tpl<a href="#" class="hintanchor" onMouseover="showhint('{$lang[cat_s_tpl_hit]}', this, event, '250px')">[?]</a></td>
    </tr>
    <tr>
        <td style="padding:4px;">{$lang['cat_f_tpl']}</td>
        <td><input class="edit" type="text" name="full_tpl">.tpl<a href="#" class="hintanchor" onMouseover="showhint('{$lang[cat_f_tpl_hit]}', this, event, '250px')">[?]</a></td>
    </tr>
    <tr>
        <td colspan="2"><div class="hr_line"></div></td>
    </tr>
		...
		if( count( $cat_info ) ) {
			...
			if( count( $root_category ) ) {
				...
				foreach ( $root_category as $id ) {
					...
					if(isset($_REQUEST['num'])) {
						$num = "&num=" . $_REQUEST['num'] . "&after";
					} else {
						$num = "";
					}
					echo "</td>
						<td class=\"list\"><nobr>[<a href=\"?mod=categories{$num}&action=edit&catid=" . $cat_info[$id]['id'] . "\">$lang[cat_ed]</a>] [<a class=maintitle href=\"?mod=categories{$num}&user_hash=" . $dle_login_hash . "&action=remove&catid=" . $cat_info[$id]['id'] . "\">$lang[cat_del]</a>]</nobr></td>
						</tr>
