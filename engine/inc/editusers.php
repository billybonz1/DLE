...
HTML;
	...
	echo <<<HTML
	<tr class="tfoot"><th colspan="10">{$npp_nav}</th></tr>
	<tr class="tfoot"><th colspan="10" align="right" valign="top">
...
<option value="mass_color">Колеровка каталоги цвета</option>
<option value="mass_discount">Скидка клиенту</option>
<option value="mass_pricelist">Прайс лист</option>
</select>
...
<table width="100%">
    <tr>
        <td bgcolor="#EFEFEF" height="29" style="padding-left:10px;"><div class="navigation">{$row['name']}</div></td>
    </tr>
</table>
<div class="unterline"></div>
<table width="100%">
    <tr>
        <td width="150" style="padding:4px;">IP:</td>
        <td><a href="#" onclick="window.opener.document.location='?mod=iptools&ip={$row['logged_ip']}'; return false;">{$row['logged_ip']}</a></td>
        <td rowspan="5" valign="top" align="right"><img src="{$avatar}" border="0" /></td>
    </tr>
    <tr>
...
<option value="3" $restricted_selected[3]>{$lang['restricted_all']}</option>
    ...
    <tr>
        ...
        <td colspan="2"><input size="20" name="fio" value="{$row['fio']}" class="edit bk"></td>
    </tr>
    <tr>
        <td style="padding:4px;">Фирма</td>
        <td colspan="2"><input size="20" name="firm" value="{$row['firm']}" class="edit bk"></td>
    </tr>
    <tr>
        <td style="padding:4px;">Вид деятельности</td>
        <td colspan="2"><input size="20" name="vid" value="{$row['vid']}" class="edit bk"></td>
    </tr>
    <tr>
        <td style="padding:4px;">Город</td>
        <td colspan="2"><input size="20" name="city" value="{$row['city']}" class="edit bk"></td>
    </tr>
    <tr>
        <td style="padding:4px;">Телефон</td>
        <td colspan="2"><input size="20" name="tel" value="{$row['tel']}" class="edit bk"></td>
    </tr>
...
elseif( $action == "doedituser" ) {
	...
	$row = $db->super_query( "SELECT user_id, user_group, email FROM " . USERPREFIX . "_users WHERE user_id = '$id'" );
    ...
    $fio = $db->safesql( $parse->BB_Parse( $parse->process( $_POST['fio'] ), false ) );
    $firm = $db->safesql( $parse->BB_Parse( $parse->process( $_POST['firm'] ), false ) );
    $vid = $db->safesql( $parse->BB_Parse( $parse->process( $_POST['vid'] ), false ) );
    $city = $db->safesql( $parse->BB_Parse( $parse->process( $_POST['city'] ), false ) );
    $tel = $db->safesql( $parse->BB_Parse( $parse->process( $_POST['tel'] ), false ) );
    
    
	$sql_update = "UPDATE " . USERPREFIX . "_users set user_group='$editlevel', banned='$banned', icq='$editicq', land='$editland', info='$editinfo', signature='$editsignature', email='$editmail', fio='$fio', firm='$firm', vid='$vid', city='$city', tel='$tel', fullname='$editfullname', time_limit='$time_limit', xfields='$filecontents'";
...
<?php
	echoheader( "users", $lang['user_head'] );
		$userId = $_REQUEST['this'];
		$resultCurrUser = $db->query("SELECT * FROM " . PREFIX . "_users WHERE user_id='$userId'");
		$myrowCurrUser = $db->get_array($resultCurrUser);
		echo "<p align='center' style='font-size:13px; font-weight:bold;'>Добавить скидку для <span style='font-size:14px; color:#0a3a6a; text-decoration:underline;'>{$myrowCurrUser[name]}</span></p>";
		echo "<form>";
			echo "<table align='center'><tr>
				  <td align='right'><p class='frmtext'>Производитель:</p></td>
				  <td>";
				echo "<select class='frmtext2' name='prz'>";
					echo "<option value=''>Выберите производителя</option>";
					$resultProizv = $db->query("SELECT * FROM " . PREFIX . "_catt ORDER BY id ASC");
					$myrowProizv = $db->get_array($resultProizv);
					
					do {
						$resultCheck = $db->query("SELECT * FROM " . PREFIX . "_disc WHERE proizv='$myrowProizv[name]' AND users_id='{$userId}'");
						$countCheck = $db->num_rows($resultCheck);
						if($countCheck == 0) {
							echo "<option value='$myrowProizv[name]'>$myrowProizv[name]</option>";
						}
					} while($myrowProizv = $db->get_array($resultProizv));
				echo "</select>";
				echo "</td></tr>
					  <tr>
					  <td><p class='frmtext'>Размер скидки:</p></td>
					  <td>";
				echo "<input type='text' name='discc' value='' class='frminput2' size='32'>";
				echo "</td></tr>";
				echo "<tr><td colspan='2' align='center'><br><br>
						...
						<input type=hidden name=user_hash value=\"{$dle_login_hash}\">
						<input type=hidden name=mod value=\"mass_user_actions\">
					  <input type='submit' class='btn' name='savenewdiscount' value='Отправить'>
					  <br><br>
					  </td></tr>";
			echo "</table>";
		echo "</form>";
