...
class ParseFilter {
	...
	function BB_Parse($source, $use_html = TRUE) {
		...
		$source = str_replace( "{comments}", "&#123;comments}", $source );
		$source = str_replace( "{addcomments}", "&#123;addcomments}", $source );
		$source = str_replace( "{navigation}", "&#123;navigation}", $source );
	...
	function font_change($tags) {
		...
		if( $type == 'size' ) {
			...
			return "<!--sizestart:{$style}--><span style=\"font-size:" . $real . "pt;line-height:100%\"><!--/sizestart-->";
		}
	...
	function build_youtube($url) {
		...
		while ($i < count($a)) {
		    $b = explode('=', $a[$i]);
		    if ($b[0] == "v") $video_link = totranslit($b[1], false);
		    $i++;
		}
	...
	function build_url($url = array()) {
		...
		if( ! preg_match( "#^(http|news|https|ed2k|ftp|aim|mms)://|(magnet:?)#", $url['html'] ) AND $url['html'][0] != "/") {
