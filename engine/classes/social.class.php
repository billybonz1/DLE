    ...
    function get_user( $social_config ) {
		...
		if (isset($token['access_token'])) {
			...
			$params = array(
				...
				'fields'       => 'id,first_name,last_name,nickname,photo_big',
			...
			if (isset($user['response'][0]['id'])) {
				...
				return array ('sid' => sha1 ('vkontakte'.$user['id']), 'nickname' => $user['nickname'], 'name' => $user['first_name'].' '.$user['last_name'], 'email' => $token['email'], 'avatar' => $user['photo_big'], 'provider' => 'vkontakte' );
