...
function check_reg_daten () {
	...
	if(document.forms.registration.fio.value == '') {

		alert("Поле 'ФИО' не может быть пустым");return false;

	}
	
	if(document.forms.registration.firm.value == '') {

		alert("Поле 'Фирма' не может быть пустым");return false;

	}
	
	if(document.forms.registration.vid.value == '') {

		alert("Вид деятельности");return false;

	}
	
	if(document.forms.registration.city.value == '') {

		alert("Поле 'Город' не может быть пустым");return false;

	}
	
	if(document.forms.registration.tel.value == '') {

		alert("Поле 'Телефон' не может быть пустым");return false;
...
if( ($doaction == "validating") and (! $stopregistration) and (! $_POST['submit_val']) ) {
	...
	if( $reg_error != "" ) {
		...
		if( ($_REQUEST['step'] != 2) and $config['registration_type'] ) {
			...
			$db->query( "INSERT INTO " . USERPREFIX . "_users (name, password, email, reg_date, lastdate, user_group, info, signature, favorites, xfields, logged_ip, fio, firm, vid, city, tel) VALUES ('$name', '$regpassword', '$email', '$add_time', '$add_time', '" . $config['reg_group'] . "', '', '', '', '', '" . $_IP . "', '" . $_REQUEST['fio'] . "','" . $_REQUEST['firm'] . "','" . $_REQUEST['vid'] . "','" . $_REQUEST['city'] . "','" . $_REQUEST['tel'] . "')" );
			$id = $db->insert_id();
