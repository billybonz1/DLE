...
if( ! $user_group[$member_id['user_group']]['allow_search'] ) {
	...
	if( intval( $config['no_date'] ) ) $this_date = " AND " . PREFIX . "_post.date < '" . $this_date . "'";
	else $this_date = "";
						...
						<legend>{$lang['s_fsoft']}</legend>
							<div style="padding:3px">
								<select name="sortby" id="sortby" class="textin" style="width:200px">
									<option value="" selected="selected">{$lang['s_fsrelate']}</option>
									<option {$sortby_sel['date']} value="date" >{$lang['s_fsdate']}</option>
...
HTML;
	...
	if( $subaction == "search" ) {
		...
		if( (empty( $story ) or ($story_maxlen < $config['search_length_min'])) and (empty( $searchuser ) or (strlen( $searchuser ) < $config['search_length_min'])) ) {
			...
			if( ! empty( $story ) ) {
				$titleonly_where = array ('0' => "MATCH(title,short_story,full_story," . PREFIX . "_post.xfields) AGAINST ('{story}')", // Искать только в статьях
										  '1' => "MATCH(text) AGAINST ('{story}')", // Искать только в комментариях
										  '2' => "MATCH(title,short_story,full_story," . PREFIX . "_post.xfields) AGAINST ('{story}')", // Искать в статьях и комментариях
										  '3' => "title LIKE '%{story}%'", // Искать только в заголовках статей
										  '5' => "id LIKE '{story}%'", // Искать по номеру статьи
										  '6' => "MATCH(" . PREFIX . "_static.template) AGAINST ('{story}')" ); // Искать только в статических страницах
			...
			if( in_array( $titleonly, array (0, 2, 3 ) ) ) {
				...
				if( $titleonly_where[$titleonly] == "" ) $titleonly_where[$titleonly] = "''";
			...
			if( $titleonly == 1 ) {
				...
				if( $titleonly_where['1'] == "" ) $titleonly_where['1'] = "''";
			...
			if( $sortby != "" ) $order_by = "ORDER BY " . $order_by;
			else $order_by = "";
			
			$sql_request = "$sql_find $order_by LIMIT $search_start,$config_search_numbers";
